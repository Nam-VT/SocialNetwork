                    ...
                    {friendsData?.content?.map(friend => (
                        <Link key={friend.id} to={`/profile/${friend.id}`} className="friend-mini-item">
                            <div className="avatar-wrapper">
                                <img
                                    src={friend.avatarUrl || `https://ui-avatars.com/api/?name=${friend.displayName?.charAt(0) || 'U'}&size=40&background=random&color=fff`}
                                    alt={friend.displayName}
                                    className="avatar-sm"
                                    onError={(e) => {
                                        e.target.onerror = null;
                                        e.target.src = `https://ui-avatars.com/api/?name=${friend.displayName?.charAt(0) || 'U'}&size=40&background=random&color=fff`;
                                    }}
                                />
